...
int32_t getPid(const uint8_t* buf)							// bufは0x47ヘッダの場所
{
...
int32_t getSectionLength(const uint8_t* buf)						// bufはTableIDの場所
{
...
void parsePmt(const uint8_t* buf, int32_t* pcrpid, int32_t* videopid, int32_t* rmpid, int32_t* rmnum, const bool bRmCap, const bool bRmDat)
{
	...
	if (bRmCap || bRmDat) {
		...
		qsort(rmpid, rmcount, sizeof(int32_t), compareForPidTable);								// 削除用PIDの昇順ソート
	}
...
int32_t compareForPidTable(const void *item1, const void *item2)							// 削除PIDテーブル qsort, bsearch 用関数
{
...
int32_t getPcrPid(const uint8_t* buf)							// bufがPMTセクションのTableIDの位置
{
...
{
	// bufが0x47の位置
	// adaptation_fieldが存在して、その長さが0でなく、PCR_flagが1ならtrue
...
int64_t getPcrValue(const uint8_t* buf)
{
	...
	const uint8_t*		pcr = buf + 6;						// pcr_clock_reference_baseの先頭
...
uint8_t* getPacketTsFileRead(TsReadProcess *ts)
{
	...
	if(remainings < ts->psize) {														// バッファ中のデータ残りがpacketsize未満になった場合, 続きのデータ読み込みを試みる
		...
		ReadFileData(ts->hFile, ts->buf, READBUFSIZE, &numRead);							// バッファにデータ読み込み
		ts->datasize = numRead;
...
//	printMsg("sync lost.\n");
	...
	const int32_t	resyncsize = ts->psize * 2 + 1 + ts->poffset;						// 3個の連続するヘッダ0x47を確認したら再同期したと見なすので、それに必要なサイズ
...
//	if(ts->bShowError) printMsg("%I64dバイトの不正なデータを削除しました.\n", brokenbyte);

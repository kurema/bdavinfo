...
{
	if (proginfo->m2tsname[0] == 0)					// .m2ts ファイル名が不明な場合（rplsファイルを開けなかった場合など）は、そのままファイル名 *****.rpls に対応する *****.m2ts とする。
	{
	...
	_snwprintf_s(proginfo->m2tsfullpath, _MAX_PATH, L"%s%s", bdav->streamdir, proginfo->m2tsname);			// m2tsファイルのフルパスを作成


	// m2tsファイルを開いてチェックする

	HANDLE	hFile = CreateFile(proginfo->m2tsfullpath, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_FLAG_SEQUENTIAL_SCAN, NULL);
	if (hFile == INVALID_HANDLE_VALUE) return false;													// m2ts ファイルを開くのに失敗した

	proginfo->m2tssize = GetFileDataSize(hFile);														// ファイルサイズ取得

	const int32_t	psize = tsFileCheck(hFile);															// ファイルチェック（188パケット, 192パケット,  コピー制御された暗号化ファイル)

	if (psize == FILE_INVALID) {																		// TSファイルではない不正なファイル（暗号化されている等）なら失敗
		CloseHandle(hFile);
		return false;
	}

	// m2tsファイルからの番組情報の取得　(192 byte packet, .m2tsファイル，パーシャルTSなので SIT に番組情報が存在する．規格外自作BDAVディスク等も考えて一応 EIT もチェックする）
...
int32_t tsFileCheck(HANDLE hReadFile)
{
	...
	return	FILE_INVALID;																													// unknown file (暗号化されている場合など）
}
...
void parseSit(const uint8_t *sitbuf, ProgInfo *proginfo, const CopyParams* param)
{
	...
	for (int32_t i = 0; i < 3; i++) proginfo->m2tsgenre[i] = -1;			// 番組ジャンル情報をクリアしておく
	...
	if ((mediaType == MEDIATYPE_CS) && ((networkID == 0x0001) || (networkID == 0x0003)))		// スカパー PerfecTV & SKY サービス (現在は使われていない、と思う)
	{
		while (i < totallen - 1)
		{
			...
			proginfo->m2tschnum = serviceID & 0x03FF;												// serviceIDからチャンネル番号を取得
...
int32_t parseEit(const uint8_t *eitbuf, ProgInfo *proginfo, const CopyParams* param)
{
	...
	proginfo->m2tschnum = ((networkID >= 0x7880) && (networkID <= 0x7FE8)) ? getTbChannelNum(networkID, serviceID, 0) : serviceID & 0x03FF;	// チャンネル番号の取得
